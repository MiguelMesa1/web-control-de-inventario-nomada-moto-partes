ALTER TABLE public.plastic_kits
  ALTER COLUMN has_headlight DROP NOT NULL,
  ALTER COLUMN has_headlight DROP DEFAULT;

UPDATE public.plastic_kits
SET has_headlight = NULL
WHERE REGEXP_REPLACE(
  LOWER(COALESCE(NULLIF(BTRIM(model), ''), brand)),
  '[^a-z0-9]+',
  '',
  'g'
) IN ('lander', 'superlander');
...
BEGIN
  ...
  IF p_parts IS NULL
    ...
    OR JSONB_ARRAY_LENGTH(p_parts) < 2 THEN
