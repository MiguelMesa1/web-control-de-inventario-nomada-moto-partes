ALTER TABLE public.plastic_kits
  ADD COLUMN has_headlight BOOLEAN NOT NULL DEFAULT FALSE;

UPDATE public.plastic_kits
SET has_headlight = TRUE
WHERE name ILIKE '%con farola%';

REVOKE EXECUTE ON FUNCTION public.save_plastic_kit(
  UUID, TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN, JSONB
) FROM PUBLIC, anon, authenticated;

DROP FUNCTION public.save_plastic_kit(
  UUID, TEXT, TEXT, TEXT, TEXT, TEXT, BOOLEAN, JSONB
);

CREATE FUNCTION public.save_plastic_kit(
  p_id UUID,
  p_name TEXT,
  p_brand TEXT,
  p_color TEXT,
...
BEGIN
  ...
  IF p_id IS NULL THEN
    ...
    VALUES (
      ...
      COALESCE(p_has_headlight, FALSE), NULLIF(BTRIM(p_model), ''),
      BTRIM(p_warehouse), COALESCE(p_active, TRUE), actor_id, actor_id
    )
  ...
  ELSE
    ...
    SET name = BTRIM(p_name),
        brand = BTRIM(p_brand),
        color = BTRIM(p_color),
        has_headlight = COALESCE(p_has_headlight, FALSE),
  ...
  VALUES (
    ...
    JSONB_BUILD_OBJECT(
      ...
      'brand', BTRIM(p_brand),
      'color', BTRIM(p_color),
      'hasHeadlight', COALESCE(p_has_headlight, FALSE),
      'warehouse', BTRIM(p_warehouse),
